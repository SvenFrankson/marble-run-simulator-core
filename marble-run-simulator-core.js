...
(function (MarbleRunSimulatorCore) {
    ...
    let Surface;
    (function (Surface) {
        Surface[Surface["None"] = 0] = "None";
        Surface[Surface["Rail"] = 1] = "Rail";
        Surface[Surface["Bowl"] = 2] = "Bowl";
        Surface[Surface["Velvet"] = 3] = "Velvet";
    ...
    class Ball extends BABYLON.Mesh {
        ...
        trailMesh;
        _boostAnimation;
        _hasBoostMaterial = false;
        _baseColor;
        _boostColor = new BABYLON.Color3(0.9, 0.1, 0.3);
        _boosting = false;
        get boosting() {
        ...
        onColliderImpact;
        rotationSpeed = 0;
        rotationAxis = BABYLON.Vector3.Right();
        surface;
        _showPositionZeroGhost = false;
        ...
        bumpSurfaceIsRail = true;
        marbleChocSound;
        railBumpSound;
        marbleLoopSound;
        marbleBowlLoopSound;
        marblePlexiglasTubeLoopSound;
        marbleInsideSound;
        _soundWorldPosition = BABYLON.Vector3.Zero();
        onMarbleChocSoundPlay;
        onRailBumpSoundPlay;
        flybackOrigin;
        flybackDestination;
        flybackPeak;
        ...
        _timer = 0;
        strReaction = 0;
        lastPosition = BABYLON.Vector3.Zero();
        visibleVelocity = BABYLON.Vector3.Zero();
        ...
        update(dt) {
            ...
            while (this._timer > 0) {
                ...
                for (let i = 0; i < this.machine.blocks.length; i++) {
                    let block = this.machine.blocks[i];
                    for (let j = 0; j < block.boxColliders.length; j++) {
                        let col = Mummu.SphereBoxIntersection(this.position, this.radius, block.boxColliders[j]);
                        //}
                        if (col.hit) {
                            //this.setLastHit(wire, col.index);
                            let colDig = col.normal.scale(-1);
...
(function (MarbleRunSimulatorCore) {
    class Block extends BABYLON.Mesh {
        ...
        updateSelectorMeshVisibility() {
            ...
            this.outlineWidth = MarbleRunSimulatorCore.UI3DConstants.outlineWidth;
            this.outlineColor = MarbleRunSimulatorCore.UI3DConstants.outlineSelectedColor;
        }
        initialize() {
            this.line = [
                new BABYLON.Vector3(0, 4 * MarbleRunSimulatorCore.tileHeight, 0),
                new BABYLON.Vector3(0, 10 * MarbleRunSimulatorCore.tileHeight, 0)
            ];
        }
        async instantiate() {
            let vertexDatas = [];
            ...
            for (let i = 0; i < this.line.length; i++) {
                ...
                let pointVData = BABYLON.CreateCylinderVertexData({ height: 1 * MarbleRunSimulatorCore.tileSize + 0.005, diameter: 0.01 });
                ...
                if (pt0 && pt1) {
                    ...
                    if (l > 0) {
                        let panelVData = BABYLON.CreateBoxVertexData({ width: 0.008, height: l + 0.002, depth: 1 * MarbleRunSimulatorCore.tileSize });
                        ...
                        let boxCollider = {
                            width: 0.005,
                            height: l,
                            depth: 3 * MarbleRunSimulatorCore.tileSize,
        ...
        intersectsMachinePart(part, i, j, k) {
            let pos = part.position;
            let d = BABYLON.Vector3.Zero();
            if (isFinite(i) && isFinite(j) && isFinite(k)) {
                pos = pos.clone();
                pos.x = i * MarbleRunSimulatorCore.tileSize;
                pos.y = k * MarbleRunSimulatorCore.tileHeight;
                pos.z = j * MarbleRunSimulatorCore.tileSize;
...
(function (MarbleRunSimulatorCore) {
    ...
    class MainMaterials {
        game;
        static UseOutlineMeshes = true;
        static SetAsOutlinedMesh(m) {
        ...
        _materialsPBR = [];
        _materialsSTD = [];
        _plexiglasMaterialsPBR = [];
        _plexiglasMaterialsSTD = [];
        ...
        _ballMaterialsPBR = [];
        _ballMaterialsSTD = [];
        _parkourBallMaterialPBR;
        _parkourBallMaterialSTD;
        ...
        baseMaterialToBallMaterialTable = [
        ...
        greenMaterial;
        blueMaterial;
        ballAnimationMaterial;
        whiteAutolitMaterial;
        whiteFullLitMaterial;
        steelFullLitMaterial;
        copperFullLitMaterial;
        get plasticBlack() {
            return this.getMaterial(6, MarbleRunSimulatorCore.MaterialQuality.Standard);
        }
        plasticWhite;
        bone;
        selectorFullLitLightBlueMaterial;
        selectorFullLitBlueMaterial;
        selectorFullLitGreenMaterial;
        constructor(game) {
            this.game = game;
...
(function (MarbleRunSimulatorCore) {
    class Wire extends BABYLON.Mesh {
        part;
        static DEBUG_DISPLAY = false;
        static DEBUG_SHOW_LENGTH = false;
        wireMesh;
        debugLengthTagMesh;
        path = [];
        normals = [];
        ...
        async instantiate(color = 0) {
            ...
            if (!Wire.DEBUG_DISPLAY) {
                ...
                if (MarbleRunSimulatorCore.MainMaterials.UseOutlineMeshes) {
                    MarbleRunSimulatorCore.MainMaterials.SetAsOutlinedMesh(this.wireMesh);
                }
...
(function (MarbleRunSimulatorCore) {
    class Track {
        ...
        _startWorldPosition = BABYLON.Vector3.Zero();
        ...
        _endWorldPosition = BABYLON.Vector3.Zero();
        ...
        AABBMin = BABYLON.Vector3.Zero();
        AABBMax = BABYLON.Vector3.Zero();
        template;
        constructor(part) {
            this.part = part;
        ...
        recomputeWiresPath(forceDisconnexion) {
            if (this instanceof MarbleRunSimulatorCore.DoubleTrack) {
...
(function (MarbleRunSimulatorCore) {
    class DoubleTrackMeshBuilder {
        static async BuildDoubleTrackMesh(track, props) {
            if (track.mesh) {
                track.mesh.dispose();
            }
            let shape = [];
...
(function (MarbleRunSimulatorCore) {
    ...
    class Machine {
        ...
        version = 12;
        dbId = -1;
        dbState = MachineDBState.Pending;
        dbLikes = 1;
        name = "Unnamed Machine";
        author = "Anonymous";
        country = "";
        isChallengeMachine = false;
        isDrawBlackboardMachine = false;
        constructionMode = MachineConstructionMode.Mode3D;
        root;
        pedestalTop;
        baseFrame;
        baseLogo;
        TEST_USE_BASE_FPS = false; // only for Poki playtest
        baseFPS;
        fpsMaterial;
        fpsTexture;
        limitsMesh;
        ...
        ready = false;
        instantiated = false;
        hasBeenOpenedInEditor = false;
        minimalAutoQualityFailed = GraphicQuality.VeryHigh + 1;
        updatingMachinePartCoordinates = false;
        playing = false;
        hasExitHole = false;
        exitShooter;
        exitTrack;
        ...
        graphicQ = GraphicQuality.Medium;
        get geometryQ() {
        ...
        onPlayCallbacks = new Nabu.UniqueList();
        play() {
            this._paused = false;
            this.playing = true;
        ...
        onStopCallbacks = new Nabu.UniqueList();
        get stopped() {
        ...
        margin = 0.05;
        baseMeshMinX = -this.margin;
        baseMeshMaxX = this.margin;
        baseMeshMinY = -this.margin;
        baseMeshMaxY = this.margin;
        baseMeshMinZ = -this.margin;
        baseMeshMaxZ = this.margin;
        ...
        serialize() {
            return MarbleRunSimulatorCore.SerializeV13(this);
        }
        static MakeMiniature(machine, data, miniatureProps) {
            if (data && (data.v === 2)) {
                let canvas = document.createElement("canvas");
                MarbleRunSimulatorCore.DeserializeV2(machine, data, true, canvas, miniatureProps);
                return canvas;
            }
            if (data && (data.v >= 3 && data.v <= 6)) {
                let canvas = document.createElement("canvas");
                MarbleRunSimulatorCore.DeserializeV3456(machine, data, true, canvas, miniatureProps);
                return canvas;
            }
            if (data && (data.v === 7 || data.v === 8)) {
                let canvas = document.createElement("canvas");
                MarbleRunSimulatorCore.DeserializeV78(machine, data, true, canvas, miniatureProps);
                return canvas;
            }
            if (data && (data.v === 9 || data.v === 10)) {
                let canvas = document.createElement("canvas");
                MarbleRunSimulatorCore.DeserializeV910(machine, data, true, canvas, miniatureProps);
                return canvas;
            }
            if (data && (data.v === 11)) {
                let canvas = document.createElement("canvas");
                MarbleRunSimulatorCore.DeserializeV11(machine, data, true, canvas, miniatureProps);
                return canvas;
            }
            if (data && (data.v === 12)) {
                let canvas = document.createElement("canvas");
                MarbleRunSimulatorCore.DeserializeV12(machine, data, true, canvas, miniatureProps);
                return canvas;
            }
            if (data && (data.v === 13)) {
                let canvas = document.createElement("canvas");
                MarbleRunSimulatorCore.DeserializeV13(machine, data, true, canvas, miniatureProps);
                return canvas;
            }
            return undefined;
        }
        lastDeserializedData;
        deserialize(data, makeMiniature) {
            ...
            if (data) {
                ...
                if (!isFinite(version) || version === 1) {
                    ...
                    return MarbleRunSimulatorCore.DeserializeV13(this, data, makeMiniature);
                }
            }
        }
        ...
        requestUpdateBaseMesh = false;
        requestUpdateShadow = false;
        updateShadow() {
...
(function (MarbleRunSimulatorCore) {
    ...
    MarbleRunSimulatorCore.MachineCollider = MachineCollider;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
class MachineName {
    static PartOnes = [
        "The ",
...
(function (MarbleRunSimulatorCore) {
    ...
    class MachinePartEndpoint {
        ...
        _absolutePosition = BABYLON.Vector3.Zero();
    ...
    class MachinePart extends BABYLON.Mesh {
        ...
        sleepersMeshes = new Map();
        selectorBodyDisplay;
        selectorBodyLogic;
        selectorEndpointsDisplay = [];
        selectorEndpointsLogic = [];
        encloseMesh;
        gridRectMesh;
        gridHeightMesh;
        isSelectable = true;
        ...
        totalLength = 0;
        globalSlope = 0;
        localBarycenter = BABYLON.Vector3.Zero();
        localBarycenterIJK = BABYLON.Vector3.Zero();
        localAABBBaseMin = new BABYLON.Vector3(-MarbleRunSimulatorCore.tileSize * 0.5, -MarbleRunSimulatorCore.tileHeight * 0.5, -MarbleRunSimulatorCore.tileSize * 0.5);
        localAABBBaseMax = new BABYLON.Vector3(MarbleRunSimulatorCore.tileSize * 0.5, MarbleRunSimulatorCore.tileHeight * 0.5, MarbleRunSimulatorCore.tileSize * 0.5);
        localAABBMin = BABYLON.Vector3.Zero();
        localAABBMax = BABYLON.Vector3.Zero();
        localRotatedAABBMin = BABYLON.Vector3.Zero();
        localRotatedAABBMax = BABYLON.Vector3.Zero();
        worldAABBMin = BABYLON.Vector3.Zero();
        worldAABBMax = BABYLON.Vector3.Zero();
        visibleWidth = 1;
        visibleHeight = 1;
        visibleDepth = 1;
        encloseStart = BABYLON.Vector3.Zero();
        enclose13 = BABYLON.Vector3.One().scaleInPlace(1 / 3);
        encloseMid = BABYLON.Vector3.One().scaleInPlace(0.5);
        enclose23 = BABYLON.Vector3.One().scaleInPlace(2 / 3);
        encloseEnd = BABYLON.Vector3.One();
        localCenter = BABYLON.Vector3.Zero();
        endPoints = [];
        ...
        neighbours = new Nabu.UniqueList();
        addNeighbour(other) {
        ...
        get mirrorable() {
            return this.template.mirrorable;
        }
        get zMirrorable() {
        ...
        offsetPosition = BABYLON.Vector3.Zero();
        targetUpdatePivot;
        _i = 0;
        _targetI;
        get i() {
        ...
        setK(v, doNotCheckGridLimits) {
            if (this._k != v) {
                this._k = v;
                if (!doNotCheckGridLimits && this.game.mode === MarbleRunSimulatorCore.GameMode.Challenge) {
                    let k = Math.min(this._k, this.game.gridKMax - Nabu.RoundTowardZero(this.localRotatedAABBMax.y / MarbleRunSimulatorCore.tileHeight));
                    k = Math.max(k, this.game.gridKMin - Nabu.RoundTowardZero(this.localRotatedAABBMin.y / MarbleRunSimulatorCore.tileHeight));
                    if (isFinite(k)) {
                        this._k = k;
                    }
                }
        ...
        _partVisibilityMode = PartVisibilityMode.Default;
        ...
        setOutlineParams(renderOutline, outlineWidth, outlineColor) {
            if (!renderOutline && MarbleRunSimulatorCore.MainMaterials.UseOutlineMeshes) {
                renderOutline = true;
                outlineWidth = 0.001;
            ...
            for (let n = 0; n < this.tracks.length; n++) {
                ...
                if (this.tracks[n].template.isPipeOrWood) {
                    ...
                    if (this.tracks[n].template.pipeIgnoresTrackNormals) {
                        points.forEach(pt => {
                            pt.y += MarbleRunSimulatorCore.PipeTrack.PIPE_OFFSET_OVER_BASETRACK;
                        });
                    }
                    else {
                        points.forEach((pt, i) => {
                            pt.addInPlace(normals[i].scale(MarbleRunSimulatorCore.PipeTrack.PIPE_OFFSET_OVER_BASETRACK));
                        });
                    }
                }
                ...
                if (MarbleRunSimulatorCore.Tools.IsWorldPosAConnexion(this.tracks[n].templateInterpolatedPoints[0])) {
                    ...
                    if (endPoint) {
                        ...
                        let endpointDisplayVertexData = BABYLON.CreateSphereVertexData({ segments: 12, diameter: 0.018, arc: 1 });
                        ...
                        if (this.visibleWidth <= 1 && this.visibleDepth <= 1) {
                            Mummu.TranslateVertexDataInPlace(endpointLogicVertexData, dir.scale(-0.009));
                        }
        ...
        refreshEncloseMeshAndLocalAABB() {
            ...
            if (this.tracks[0] && this.tracks[0].template.isWood) {
                this.localAABBMax.y += MarbleRunSimulatorCore.tileSize;
            }
            this.visibleWidth = Math.round((this.localAABBMax.x - this.localAABBMin.x) / MarbleRunSimulatorCore.tileSize);
            this.visibleHeight = Math.round((this.localAABBMax.y - this.localAABBMin.y) / MarbleRunSimulatorCore.tileHeight);
            this.visibleDepth = Math.round((this.localAABBMax.z - this.localAABBMin.z) / MarbleRunSimulatorCore.tileSize);
            this.localBarycenter = new BABYLON.Vector3((this.localAABBMax.x + this.localAABBMin.x) * 0.5, (this.localAABBMax.y + this.localAABBMin.y) * 0.5, (this.localAABBMax.z + this.localAABBMin.z) * 0.5);
                ...
                for (let i = 0; i < this.template.trackTemplates.length; i++) {
                    ...
                    if (!track) {
                        if (this.template.trackTemplates[i].isPipe) {
                            ...
                            track = new MarbleRunSimulatorCore.DoubleTrack(this);
                        }
                        else {
        ...
        rebuildWireMeshes(rebuildNeighboursWireMeshes, skipSleepersAndSupport) {
            ...
            this.tracks.forEach((track) => {
                ...
                if (track instanceof MarbleRunSimulatorCore.PipeTrack) {
                    ...
                    MarbleRunSimulatorCore.DoubleTrackMeshBuilder.BuildDoubleTrackMesh(track, {});
                }
                else {
        ...
        doSleepersMeshUpdate() {
            ...
            datas.forEach((vData, colorIndex) => {
                if (!this.sleepersMeshes.get(colorIndex)) {
                    ...
                    if (MarbleRunSimulatorCore.MainMaterials.UseOutlineMeshes) {
                        MarbleRunSimulatorCore.MainMaterials.SetAsOutlinedMesh(sleeperMesh);
                    }
    ...
    class MachinePartFactory {
        ...
        createTrack(partName, prop) {
            ...
            if (partName === "pipeunderloop" || partName.startsWith("pipeunderloop_")) {
                ...
                prop.pipeVersion = true;
                return new MarbleRunSimulatorCore.Loop(this.machine, prop, true);
            }
            ...
            if (partName === "controlerLegacy") {
                return new MarbleRunSimulatorCore.Controler_Legacy(this.machine, prop);
            }
            ...
            if (partName === "tsplit") {
                return new MarbleRunSimulatorCore.TSplit(this.machine, prop);
            }
            if (partName === "tjoin") {
                return new MarbleRunSimulatorCore.TJoin(this.machine, prop);
            }
            if (partName === "cross2d") {
                return new MarbleRunSimulatorCore.Cross2D(this.machine, prop);
            }
            if (partName === "bitsplit") {
                return new MarbleRunSimulatorCore.BitSplit(this.machine, prop);
            }
            if (partName === "largesplit") {
                return new MarbleRunSimulatorCore.LargeBitSplit(this.machine, prop);
            }
            if (partName === "dropside" || partName.startsWith("dropside_")) {
                ...
                return new MarbleRunSimulatorCore.DropSide(this.machine, prop);
            }
            if (partName === "dropback" || partName.startsWith("dropback_")) {
                ...
                return new MarbleRunSimulatorCore.DropBack(this.machine, prop);
            }
            if (partName === "diamond" || partName.startsWith("diamond_")) {
                ...
                return new MarbleRunSimulatorCore.Diamond(this.machine, prop);
            }
            if (partName === "box" || partName.startsWith("box_")) {
                ...
                return new MarbleRunSimulatorCore.Box(this.machine, prop);
            }
...
(function (MarbleRunSimulatorCore) {
    class PipeTrack extends MarbleRunSimulatorCore.Track {
        static PIPE_OFFSET_OVER_BASETRACK = 0.0085;
        ringsMesh;
        tubeMesh;
        tubePath = [];
        get preferedStartBank() {
    ...
    class SleeperMeshBuilder {
        static GenerateSleepersVertexData(part, props) {
            ...
            for (let j = 0; j < part.tracks.length; j++) {
                ...
                if (part.game.materials.getMaterialType(colorIndex) === MarbleRunSimulatorCore.MaterialType.Plastic && MarbleRunSimulatorCore.MainMaterials.UseOutlineMeshes) {
...
(function (MarbleRunSimulatorCore) {
    ...
    class MachinePartTemplate {
        partName = "machine-part-template";
        l = 1;
        h = 1;
        d = 1;
        n = 1;
        ...
        maxAngle = Math.PI / 4;
        minTurnRadius = 0.06;
        lExtendableOnX = false;
        lExtendableOnXZ = false;
        lExtendableOnZ = false;
        hExtendableOnY = false;
        dExtendableOnZ = false;
        xExtendable = false;
        yExtendable = false;
        downwardYExtendable = false;
        zExtendable = false;
        nExtendable = false;
        sExtendable = false;
        minLAbsolute = 0;
        ...
        mirrorable = false;
        zMirrorable = false;
        hasOriginDestinationHandles = false;
        getWidthForDepth;
        getWidthForHeight;
        getDepthForWidth;
        trackTemplates = [];
    ...
    class TemplateManager {
        ...
        getTemplate(partName, mirror, mirrorZ) {
            ...
            if (!data) {
                if (partName.startsWith("curb_")) {
                    ...
                    data = MarbleRunSimulatorCore.Ramp.GenerateTemplate(w, h, isFinite(d) ? d : 1, s, mirror, mirrorZ);
                }
                    ...
                    data = MarbleRunSimulatorCore.Snake.GenerateTemplate(w, s);
                }
                    ...
                    let h = parseInt(partName.split("_")[1]);
                    data = MarbleRunSimulatorCore.SteamElevator.GenerateTemplate(h, mirror);
                }
                    ...
                    data = MarbleRunSimulatorCore.Sort.GenerateTemplate(mirror, mirrorZ);
                }
                else if (partName === "controlerLegacy") {
                    data = MarbleRunSimulatorCore.Controler_Legacy.GenerateTemplate(mirror);
                }
                else if (partName === "controler") {
                    data = MarbleRunSimulatorCore.Controler.GenerateTemplate(mirror);
                }
                else if (partName === "spawner") {
                    data = MarbleRunSimulatorCore.Spawner.GenerateTemplate(mirror);
                }
                    ...
                    let l = parseInt(partName.split("_")[1].split(".")[0]);
                    data = MarbleRunSimulatorCore.MultiJoin.GenerateTemplate(l, mirror);
                }
                    ...
                    let d = parseInt(partName.split("_")[1].split(".")[1]);
                    let n = parseInt(partName.split("_")[1].split(".")[2]);
                    data = MarbleRunSimulatorCore.Loop.GenerateTemplate(l, d, n, true);
                }
                else if (partName.startsWith("pipeunderloop_")) {
                    let l = parseInt(partName.split("_")[1].split(".")[0]);
                    let d = parseInt(partName.split("_")[1].split(".")[1]);
                    let n = parseInt(partName.split("_")[1].split(".")[2]);
                    data = MarbleRunSimulatorCore.Loop.GenerateTemplate(l, d, n, true, true);
                }
                    ...
                    let l = parseInt(partName.split("_")[1].split(".")[0]);
                    let h = parseInt(partName.split("_")[1].split(".")[1]);
                    data = MarbleRunSimulatorCore.Coil.GenerateTemplate(l, h, mirror);
                }
                else if (partName === "quarter") {
                    data = MarbleRunSimulatorCore.QuarterNote.GenerateTemplate(mirror);
                }
                else if (partName === "double") {
                    data = MarbleRunSimulatorCore.DoubleNote.GenerateTemplate(mirror);
                }
                else if (partName === "start") {
                    data = MarbleRunSimulatorCore.Start.GenerateTemplate();
                }
                else if (partName === "end") {
                    data = MarbleRunSimulatorCore.End.GenerateTemplate(mirror);
                }
                    ...
                    data = MarbleRunSimulatorCore.GravityWell.GenerateTemplate(mirror);
                }
                    ...
                    data = MarbleRunSimulatorCore.TSplit.GenerateTemplate(mirror);
                }
                else if (partName === "tjoin") {
                    data = MarbleRunSimulatorCore.TJoin.GenerateTemplate();
                }
                else if (partName === "cross2d") {
                    data = MarbleRunSimulatorCore.Cross2D.GenerateTemplate();
                }
                else if (partName === "bitsplit") {
                    data = MarbleRunSimulatorCore.BitSplit.GenerateTemplate(mirror);
                }
                else if (partName === "largesplit") {
                    data = MarbleRunSimulatorCore.LargeBitSplit.GenerateTemplate(mirror);
                }
                else if (partName.startsWith("dropside_")) {
                    let h = parseInt(partName.split("_")[1].split(".")[0]);
                    data = MarbleRunSimulatorCore.DropSide.GenerateTemplate(h, mirror);
                }
                else if (partName.startsWith("dropback_")) {
                    let h = parseInt(partName.split("_")[1].split(".")[0]);
                    data = MarbleRunSimulatorCore.DropBack.GenerateTemplate(h);
                }
                else if (partName.startsWith("diamond_")) {
                    let l = parseInt(partName.split("_")[1].split(".")[0]);
                    data = MarbleRunSimulatorCore.Diamond.GenerateTemplate(l);
                }
                else if (partName.startsWith("box_")) {
                    let l = parseInt(partName.split("_")[1].split(".")[0]);
                    let h = parseInt(partName.split("_")[1].split(".")[1]);
                    data = MarbleRunSimulatorCore.Box.GenerateTemplate(l, h);
                }
        ...
        getTemplateByProp(baseName, prop) {
            ...
            if (baseName === "curb") {
                ...
                partName = MarbleRunSimulatorCore.Controler_Legacy.PropToPartName(prop);
            }
            else if (baseName === "controler") {
                partName = MarbleRunSimulatorCore.Controler.PropToPartName(prop);
            }
                ...
                prop.pipeVersion = true;
                partName = MarbleRunSimulatorCore.Loop.PropToPartName(prop, true);
            }
                ...
                partName = MarbleRunSimulatorCore.EndBasket.PropToPartName(prop);
            }
            else if (baseName === "blackboard") {
                partName = MarbleRunSimulatorCore.BlackBoard.PropToPartName(prop);
            }
                ...
                partName = MarbleRunSimulatorCore.TSplit.PropToPartName(prop);
            }
            else if (baseName === "tjoin") {
                partName = MarbleRunSimulatorCore.TJoin.PropToPartName(prop);
            }
            else if (baseName === "cross2d") {
                partName = MarbleRunSimulatorCore.Cross2D.PropToPartName(prop);
            }
            else if (baseName === "bitsplit") {
                partName = MarbleRunSimulatorCore.BitSplit.PropToPartName(prop);
            }
            else if (baseName === "largesplit") {
                partName = MarbleRunSimulatorCore.LargeBitSplit.PropToPartName(prop);
            }
            else if (baseName === "dropside") {
                partName = MarbleRunSimulatorCore.DropSide.PropToPartName(prop);
            }
            else if (baseName === "dropback") {
                partName = MarbleRunSimulatorCore.DropBack.PropToPartName(prop);
            }
            else if (baseName === "diamond") {
                partName = MarbleRunSimulatorCore.Diamond.PropToPartName(prop);
            }
            else if (baseName === "box") {
                partName = MarbleRunSimulatorCore.Box.PropToPartName(prop);
            }
...
(function (MarbleRunSimulatorCore) {
    class TrackPoint {
        ...
        fixedNormal = false;
        fixedDir = false;
        fixedTangentIn = false;
        fixedTangentOut = false;
        summedLength = 0;
...
(function (MarbleRunSimulatorCore) {
    class UI3DConstants {
        static logicColliderVisibility = 0.2;
        static logicColliderIsVisible = false;
        static outlineWidth = 0.0015;
        static outlineBaseColor = new BABYLON.Color3(0.4, 0.4, 0.4);
        static outlineHoverColor = new BABYLON.Color3(0.7, 0.7, 0.7);
        static outlineSelectedColor = new BABYLON.Color3(1, 1, 1);
    }
...
(function (MarbleRunSimulatorCore) {
    ...
    function DrawMiniature(lines, canvas, data, miniatureProps) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line instanceof MarbleRunSimulatorCore.MiniatureTrack && line.isPipe) {
                context.lineWidth += 4;
            }
            let normalizedH = 0;
            context.beginPath();
...
(function (MarbleRunSimulatorCore) {
    ...
    function DeserializeV11(machine, data, makeMiniature = false, canvas, miniatureProps) {
...
(function (MarbleRunSimulatorCore) {
    ...
    function DeserializeV12(machine, data, makeMiniature = false, canvas, miniatureProps) {
        let dataString = data.d;
        if (!dataString) {
            dataString = data.content;
        }
...
(function (MarbleRunSimulatorCore) {
    function SerializeV13(machine) {
        ...
        dataString += MarbleRunSimulatorCore.NToHex(machine.blocks.length, 2);
        for (let i = 0; i < machine.blocks.length; i++) {
            let block = machine.blocks[i];
            dataString += MarbleRunSimulatorCore.NToHex(block.line.length, 2);
            for (let j = 0; j < block.line.length; j++) {
                let x = Math.round(block.line[j].x * 1000) + MarbleRunSimulatorCore.ballOffset;
                let y = Math.round(block.line[j].y * 1000) + MarbleRunSimulatorCore.ballOffset;
                let z = Math.round(block.line[j].z * 1000) + MarbleRunSimulatorCore.ballOffset;
                dataString += MarbleRunSimulatorCore.NToHex(x, 3);
                dataString += MarbleRunSimulatorCore.NToHex(y, 3);
                dataString += MarbleRunSimulatorCore.NToHex(z, 3);
            }
    ...
    MarbleRunSimulatorCore.SerializeV13 = SerializeV13;
    function DeserializeV13(machine, data, makeMiniature = false, canvas, miniatureProps) {
        let dataString = data.d;
        if (!dataString) {
            dataString = data.content;
        }
        if (dataString) {
            ...
            for (let i = 0; i < blocksCount; i++) {
                ...
                let block;
                if (machine) {
                    block = new MarbleRunSimulatorCore.Block(machine);
    ...
    MarbleRunSimulatorCore.DeserializeV13 = DeserializeV13;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    ...
    function DeserializeV2(machine, data, makeMiniature = false, canvas, miniatureProps) {
        let dataString = data.d;
        if (!dataString) {
            dataString = data.content;
        }
...
(function (MarbleRunSimulatorCore) {
    ...
    function DeserializeV3456(machine, data, makeMiniature = false, canvas, miniatureProps) {
        let dataString = data.d;
        if (!dataString) {
            dataString = data.content;
        }
...
(function (MarbleRunSimulatorCore) {
    ...
    function DeserializeV78(machine, data, makeMiniature = false, canvas, miniatureProps) {
        let dataString = data.d;
        if (!dataString) {
            dataString = data.content;
        }
...
(function (MarbleRunSimulatorCore) {
    ...
    function DeserializeV910(machine, data, makeMiniature = false, canvas, miniatureProps) {
        let dataString = data.d;
        if (!dataString) {
            dataString = data.content;
        }
...
(function (MarbleRunSimulatorCore) {
    class Xylophone extends MarbleRunSimulatorCore.MachineDecor {
        ...
        trigger;
        blade;
        _animateTrigger = Mummu.AnimationFactory.EmptyNumberCallback;
        _animateTriggerBack = Mummu.AnimationFactory.EmptyNumberCallback;
        get noteLetterIndex() {
...
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class BitSplit extends MarbleRunSimulatorCore.MachinePart {
        _animatePivot = Mummu.AnimationFactory.EmptyNumberCallback;
        pivot;
        axisZMin = 0;
        axisZMax = 1;
        clicSound;
        ...
        static GenerateTemplate(mirror) {
            ...
            template.trackTemplates[0].trackpoints = [
                ...
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(0, 0.002, 0), dir),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(MarbleRunSimulatorCore.tileSize * 0.5, 0, 0), dir)
            ];
            template.initialize();
            return template;
...
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class LargeBitSplit extends MarbleRunSimulatorCore.MachinePart {
        _animatePivot = Mummu.AnimationFactory.EmptyNumberCallback;
        pivot;
        stepLeft;
        stepRight;
        ...
        axisZMax = 1;
        clicSound;
        static boxRadius = 0.9 * MarbleRunSimulatorCore.tileSize;
        constructor(machine, prop) {
            super(machine, prop);
            ...
            this.pivot = new BABYLON.Mesh("pivot");
            if (MarbleRunSimulatorCore.MainMaterials.UseOutlineMeshes) {
                MarbleRunSimulatorCore.MainMaterials.SetAsOutlinedMesh(this.pivot);
            }
            ...
            let wireLeftP = new MarbleRunSimulatorCore.Wire(this);
            wireLeftP.wireSize = 0.006;
            wireLeftP.path = [];
            wireLeftP.position.copyFrom(this.pivot.position);
            wireLeftP.parent = this;
            let wireLeftM = new MarbleRunSimulatorCore.Wire(this);
            ...
            let wireRightP = new MarbleRunSimulatorCore.Wire(this);
            wireRightP.wireSize = 0.006;
            wireRightP.path = [];
            wireRightP.position.copyFrom(this.pivot.position);
            wireRightP.parent = this;
            let wireRightM = new MarbleRunSimulatorCore.Wire(this);
            ...
            let wireBottomP = new MarbleRunSimulatorCore.Wire(this);
            wireBottomP.wireSize = 0.006;
            wireBottomP.path = [];
            wireBottomP.position.copyFrom(this.pivot.position);
            wireBottomP.parent = this;
            let wireBottomM = new MarbleRunSimulatorCore.Wire(this);
            ...
            this.localAABBBaseMax.x = 1.5 * MarbleRunSimulatorCore.tileSize;
            this.localAABBBaseMax.y = 1.5 * MarbleRunSimulatorCore.tileHeight;
            this._animatePivot = Mummu.AnimationFactory.CreateNumber(this, this.pivot.rotation, "z", () => {
                if (!this.machine.playing) {
                    this.pivot.rotation.z = (this.mirrorX ? -1 : 1) * Math.PI / 4;
                }
    ...
    MarbleRunSimulatorCore.LargeBitSplit = LargeBitSplit;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class BlackBoardPiece extends BABYLON.Mesh {
        ...
        wFactor;
        hFactor;
        constructor(blackboard, wFactor, hFactor) {
    ...
    class BlackBoard extends MarbleRunSimulatorCore.MachinePart {
        ...
        constructor(machine, prop) {
            ...
            let rawPoints = [];
            for (let i = 0; i < 10; i++) {
                let x = (i + 0.5 * Math.random()) * MarbleRunSimulatorCore.tileSize;
                let y = (9 - i * 0.4 + Math.random()) * MarbleRunSimulatorCore.tileHeight;
            ...
            if (MarbleRunSimulatorCore.MainMaterials.UseOutlineMeshes) {
                MarbleRunSimulatorCore.MainMaterials.SetAsOutlinedMesh(this.borders[1]);
            }
            ...
            if (MarbleRunSimulatorCore.MainMaterials.UseOutlineMeshes) {
                MarbleRunSimulatorCore.MainMaterials.SetAsOutlinedMesh(this.borders[2]);
            }
            ...
            if (MarbleRunSimulatorCore.MainMaterials.UseOutlineMeshes) {
                MarbleRunSimulatorCore.MainMaterials.SetAsOutlinedMesh(this.borders[3]);
            }
        ...
        regenerateTemplate() {
            ...
            for (let n = 0; n < this.lines.length; n++) {
                ...
                for (let i = 0; i < rawLine.length; i++) {
                    ...
                    trackTemplate.trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(trackTemplate, rawPoint, dir, norm));
        ...
        clampLocalPositionOnBBox(pt, radius) {
            let clampedPt = pt.clone();
            clampedPt.x = Nabu.MinMax(clampedPt.x, -MarbleRunSimulatorCore.tileSize * 0.5 + radius, (this.w - 0.5) * MarbleRunSimulatorCore.tileSize - radius);
            clampedPt.y = Nabu.MinMax(clampedPt.y, -MarbleRunSimulatorCore.tileHeight * 0.5 + radius, (this.h - 0.5) * MarbleRunSimulatorCore.tileHeight - radius);
            return clampedPt;
        }
...
(function (MarbleRunSimulatorCore) {
    class Coil extends MarbleRunSimulatorCore.MachinePart {
        constructor(machine, prop) {
            ...
            let z0 = -Math.floor((this.l - 1) * 0.5) * MarbleRunSimulatorCore.tileSize;
            ...
            for (let n = 0; n <= count + 2; n++) {
                for (let i = 0; i < 64; i++) {
                    ...
                    drop = Math.min(drop, MarbleRunSimulatorCore.tileHeight * 0.3);
                    y = Math.max(y, -h) - drop;
                    coilWire.path.push(new BABYLON.Vector3(0.5 * (x0 + x1) + x, y, zSign * (z0 + z)));
                }
            }
        ...
        static GenerateTemplate(l, h, mirror) {
            let template = new MarbleRunSimulatorCore.MachinePartTemplate();
            ...
            let c = (x0 + x1) * 0.5;
            let z0 = -Math.floor((template.l - 1) * 0.5) * MarbleRunSimulatorCore.tileSize;
            let z1 = 0.014 * 0.3;
            let distZ = z1 - (z0 + 0.014 * 0.7);
            let r0 = z1 - z0;
            let coilH = (template.h - 1) * MarbleRunSimulatorCore.tileHeight;
            ...
            template.trackTemplates[0].trackpoints = [
                ...
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3((x0 + x1) * 0.5, -0.007 * sinB, 0.007 * (1 - cosB) + z1 - 0.014 * 0.5), MarbleRunSimulatorCore.Tools.V3Dir(template.l > 0 ? 90 : -90), n)
            ];
            template.trackTemplates[1] = new MarbleRunSimulatorCore.TrackTemplate(template);
            template.trackTemplates[1].colorIndex = 1;
            template.trackTemplates[1].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(c - MarbleRunSimulatorCore.tileSize, -MarbleRunSimulatorCore.tileHeight * (template.h + 0) + 0.01, z0), MarbleRunSimulatorCore.Tools.V3Dir(125), undefined, undefined, 0.7),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(c + MarbleRunSimulatorCore.tileSize, -MarbleRunSimulatorCore.tileHeight * (template.h + 0), z0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(x1, -MarbleRunSimulatorCore.tileHeight * (template.h + 0), z0), MarbleRunSimulatorCore.Tools.V3Dir(90))
            ];
...
(function (MarbleRunSimulatorCore) {
    class Controler extends MarbleRunSimulatorCore.MachinePart {
        ...
        constructor(machine, prop) {
            ...
            this.axisZMin = -this.wireGauge * 0.6 - depth * MarbleRunSimulatorCore.tileSize;
            ...
            this.support.position.copyFromFloats(0, -MarbleRunSimulatorCore.tileHeight * 0.5, -depth / 2 * MarbleRunSimulatorCore.tileSize);
            this.support.parent = this;
            let wireVertical0 = new MarbleRunSimulatorCore.Wire(this);
            wireVertical0.colorIndex = 5;
            wireVertical0.parent = this.pivotPass;
            wireVertical0.path = [new BABYLON.Vector3(0, Controler.pivotL, -dz - depth * MarbleRunSimulatorCore.tileSize), new BABYLON.Vector3(0, -Controler.pivotL, -dz - depth * MarbleRunSimulatorCore.tileSize)];
            let wireVertical1 = new MarbleRunSimulatorCore.Wire(this);
            wireVertical1.colorIndex = 5;
            wireVertical1.parent = this.pivotPass;
            wireVertical1.path = [new BABYLON.Vector3(0, Controler.pivotL, dz - depth * MarbleRunSimulatorCore.tileSize), new BABYLON.Vector3(0, -Controler.pivotL, dz - depth * MarbleRunSimulatorCore.tileSize)];
        ...
        async instantiateMachineSpecific() {
            ...
            let axisPassVertexData = BABYLON.CreateCylinderVertexData({ height: MarbleRunSimulatorCore.tileSize * depth / 2 + this.wireGauge * 1.2, diameter: 0.001 });
            Mummu.RotateVertexDataInPlace(axisPassVertexData, q);
            Mummu.TranslateVertexDataInPlace(axisPassVertexData, new BABYLON.Vector3(0, 0, -0.5 * MarbleRunSimulatorCore.tileSize * depth / 2));
            let axisControllerVertexData = BABYLON.CreateCylinderVertexData({ height: MarbleRunSimulatorCore.tileSize * depth / 2 + this.wireGauge * 1.2, diameter: 0.001 });
            Mummu.RotateVertexDataInPlace(axisControllerVertexData, q);
            Mummu.TranslateVertexDataInPlace(axisControllerVertexData, new BABYLON.Vector3(0, MarbleRunSimulatorCore.tileHeight, 0.5 * MarbleRunSimulatorCore.tileSize * depth / 2));
            ...
            Mummu.TranslateVertexDataInPlace(cog8Data, new BABYLON.Vector3(0, 0, -depth / 2 * MarbleRunSimulatorCore.tileSize));
            ...
            Mummu.TranslateVertexDataInPlace(cog13Data, new BABYLON.Vector3(0, 0, -depth / 2 * MarbleRunSimulatorCore.tileSize));
        ...
        static GenerateTemplate(mirrorX) {
            ...
            template.trackTemplates[3].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[3], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileWidth * 0.5, 0, -depth * MarbleRunSimulatorCore.tileSize), dir),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[3], pEndLeft.subtract(dirEnd.scale(0.0005)).subtractFromFloats(0, 0, depth * MarbleRunSimulatorCore.tileSize), dirEnd)
            ];
            // Pass out Left
            ...
            template.trackTemplates[4].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[4], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileWidth * 0.5, -MarbleRunSimulatorCore.tileHeight * template.h, -depth * MarbleRunSimulatorCore.tileSize), dir),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[4], new BABYLON.Vector3(-MarbleRunSimulatorCore.Split.pivotL / Math.SQRT2, -MarbleRunSimulatorCore.tileHeight * 0.5 - MarbleRunSimulatorCore.Split.pivotL / Math.SQRT2 - 0.001, -depth * MarbleRunSimulatorCore.tileSize), dirEnd.multiplyByFloats(1, -1, 1))
            ];
            // Pass out Right
            ...
            template.trackTemplates[5].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[5], new BABYLON.Vector3(MarbleRunSimulatorCore.Split.pivotL / Math.SQRT2, -MarbleRunSimulatorCore.tileHeight * 0.5 - MarbleRunSimulatorCore.Split.pivotL / Math.SQRT2 - 0.001, -depth * MarbleRunSimulatorCore.tileSize), dirEnd),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[5], new BABYLON.Vector3(MarbleRunSimulatorCore.tileWidth * 0.5, -MarbleRunSimulatorCore.tileHeight * template.h, -depth * MarbleRunSimulatorCore.tileSize), dir)
            ];
            // Shield
            ...
            template.trackTemplates[6].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[6], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileWidth * 0.25, 0.016, -depth * MarbleRunSimulatorCore.tileSize), MarbleRunSimulatorCore.Tools.V3Dir(100), new BABYLON.Vector3(0, -1, 0)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[6], new BABYLON.Vector3(0, 0.005, -depth * MarbleRunSimulatorCore.tileSize)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[6], new BABYLON.Vector3(MarbleRunSimulatorCore.tileWidth * 0.25, 0.016, -depth * MarbleRunSimulatorCore.tileSize), MarbleRunSimulatorCore.Tools.V3Dir(80), new BABYLON.Vector3(0, -1, 0)),
            ];
...
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class Controler_Legacy extends MarbleRunSimulatorCore.MachinePart {
        _animatePivot = Mummu.AnimationFactory.EmptyNumberCallback;
        pivotPass;
        pivotController;
        pivotControllerCollider;
        ...
        constructor(machine, prop) {
            ...
            wireVertical0.colorIndex = 5;
            wireVertical0.parent = this.pivotPass;
            wireVertical0.path = [new BABYLON.Vector3(0, Controler_Legacy.pivotL, -dz - depth * MarbleRunSimulatorCore.tileSize), new BABYLON.Vector3(0, -Controler_Legacy.pivotL, -dz - depth * MarbleRunSimulatorCore.tileSize)];
            let wireVertical1 = new MarbleRunSimulatorCore.Wire(this);
            wireVertical1.colorIndex = 5;
            wireVertical1.parent = this.pivotPass;
            wireVertical1.path = [new BABYLON.Vector3(0, Controler_Legacy.pivotL, dz - depth * MarbleRunSimulatorCore.tileSize), new BABYLON.Vector3(0, -Controler_Legacy.pivotL, dz - depth * MarbleRunSimulatorCore.tileSize)];
            this.wires = [wireVertical0, wireVertical1];
            this.generateWires();
            this._animatePivot = Mummu.AnimationFactory.CreateNumber(this, this.pivotPass.rotation, "z", () => {
                if (!this.machine.playing) {
    ...
    MarbleRunSimulatorCore.Controler_Legacy = Controler_Legacy;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    ...
    MarbleRunSimulatorCore.Cross2D = Cross2D;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
...
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class DropBack extends MarbleRunSimulatorCore.MachinePart {
        shieldConnector;
        constructor(machine, prop) {
            super(machine, prop);
            this.setColorCount(2);
            ...
            let upperShieldWire = new MarbleRunSimulatorCore.Wire(this);
        ...
        static GenerateTemplate(h) {
            ...
            template.trackTemplates[0].colorIndex = 0;
            template.trackTemplates[0].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileSize * 0.5, 0, 0), new BABYLON.Vector3(1, 0, 0)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(-r + 0.002, 0, 0), new BABYLON.Vector3(1, 0, 0))
            ];
            template.trackTemplates[1] = new MarbleRunSimulatorCore.TrackTemplate(template);
            template.trackTemplates[1].colorIndex = 1;
            template.trackTemplates[1].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(r, -MarbleRunSimulatorCore.tileHeight * template.h + 0.004, 0), (new BABYLON.Vector3(-1, -0.4, 0)).normalize()),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileSize * 0.5, -MarbleRunSimulatorCore.tileHeight * template.h, 0), new BABYLON.Vector3(-1, 0, 0))
            ];
            template.trackTemplates[1].drawStartTip = true;
            template.maxAngle = 0;
    ...
    MarbleRunSimulatorCore.DropBack = DropBack;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class DropSide extends MarbleRunSimulatorCore.MachinePart {
        shieldConnector;
        constructor(machine, prop) {
            super(machine, prop);
            this.setColorCount(2);
            this.setTemplate(this.machine.templateManager.getTemplate(DropSide.PropToPartName(prop), prop.mirrorX));
            let rShield = 0.009;
            let hShield = 0.007;
            let shieldWire = new MarbleRunSimulatorCore.Wire(this);
            shieldWire.colorIndex = 0;
        ...
        static PropToPartName(prop) {
            let partName = "dropside_" + prop.h.toFixed(0);
            return partName;
        }
        async instantiateMachineSpecific() {
            this.shieldConnector.material = this.game.materials.getMaterial(this.getColor(0), this.machine.materialQ);
        }
        static GenerateTemplate(h, mirror) {
            let template = new MarbleRunSimulatorCore.MachinePartTemplate();
            template.partName = "dropside_" + h.toFixed(0);
            template.h = h;
            template.hExtendableOnY = true;
            template.downwardYExtendable = true;
            template.minH = 1;
    ...
    MarbleRunSimulatorCore.DropSide = DropSide;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    ...
    class End extends MarbleRunSimulatorCore.MachinePart {
        ...
        constructor(machine, prop) {
            ...
            for (let i = 0; i < 32; i++) {
                ...
                shieldWireR.path.push(new BABYLON.Vector3(x, MarbleRunSimulatorCore.tileHeight, z));
            }
        ...
        static GenerateTemplate(mirrorX) {
            ...
            template.trackTemplates[1].trackpoints = [
                ...
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(x0 - 0.001, y0 - 0.002, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(x0, y0 - 0.002, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(x0 + 0.001, y0 - 0.002, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(x0 + 0.012, y0, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(x0 + w - r, y0, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                ...
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(x0 + w, 2 * MarbleRunSimulatorCore.tileHeight, 0), MarbleRunSimulatorCore.Tools.V3Dir(0), MarbleRunSimulatorCore.Tools.V3Dir(-90)),
            ];
            template.trackTemplates[1].drawStartTip = true;
...
(function (MarbleRunSimulatorCore) {
    class EndBasket extends MarbleRunSimulatorCore.MachinePart {
        ...
        constructor(machine, prop) {
            ...
            let d = 3 * MarbleRunSimulatorCore.tileSize;
            ...
            if (MarbleRunSimulatorCore.MainMaterials.UseOutlineMeshes) {
                MarbleRunSimulatorCore.MainMaterials.SetAsOutlinedMesh(this.flagPole);
            }
            ...
            if (MarbleRunSimulatorCore.MainMaterials.UseOutlineMeshes) {
                MarbleRunSimulatorCore.MainMaterials.SetAsOutlinedMesh(this.flag);
            }
    ...
    MarbleRunSimulatorCore.EndBasket = EndBasket;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
...
(function (MarbleRunSimulatorCore) {
    class GravityWell extends MarbleRunSimulatorCore.MachinePart {
        ...
        static GenerateTemplate(mirrorX) {
            ...
            for (let n = 0; n <= 7; n++) {
                let a = n * Math.PI / 4;
                let x = Math.sin(a) * MarbleRunSimulatorCore.tileWidth;
                let z = Math.cos(a) * MarbleRunSimulatorCore.tileWidth;
                let f = n / 7;
                let dir = undefined;
                ...
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(MarbleRunSimulatorCore.tileWidth * 0.5 + x, -0.007 * n / 7, -MarbleRunSimulatorCore.tileDepth + z), dir, norm));
            }
            template.trackTemplates[0].drawEndTip = true;
            template.trackTemplates[1] = new MarbleRunSimulatorCore.TrackTemplate(template);
            template.trackTemplates[1].colorIndex = 1;
            template.trackTemplates[1].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileWidth * 0.2 + MarbleRunSimulatorCore.tileWidth * 0.5, -MarbleRunSimulatorCore.tileHeight * template.h + 0.01, -MarbleRunSimulatorCore.tileDepth), MarbleRunSimulatorCore.Tools.V3Dir(125), undefined, undefined, 0.7),
            ...
            template.initialize();
            let wellPath = [new BABYLON.Vector3(0.012, 0, 0), new BABYLON.Vector3(MarbleRunSimulatorCore.tileWidth, MarbleRunSimulatorCore.tileHeight * 0.9, 0)];
    ...
    class Jumper extends MarbleRunSimulatorCore.MachinePart {
        ...
        static GenerateTemplate(n) {
            ...
            if (n < 9) {
                ...
                let d = 0.015;
                let x0 = -MarbleRunSimulatorCore.tileWidth * 0.5 + d;
                let y0 = -MarbleRunSimulatorCore.tileHeight * 2;
                let radius = MarbleRunSimulatorCore.tileSize * 3 - d;
                template.trackTemplates[0] = new MarbleRunSimulatorCore.TrackTemplate(template);
                template.trackTemplates[0].trackpoints = [
                    new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileWidth * 0.5, y0, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                    new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(x0, y0, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                    new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(x0 + radius * Math.sin(Math.PI / 6), y0 + radius * (1 - Math.cos(Math.PI / 6)), 0)),
                    new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(x0 + radius * Math.sin(2 * Math.PI / 6), y0 + radius * (1 - Math.cos(2 * Math.PI / 6)), 0)),
                    new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(x0 + radius, y0 + radius, 0), MarbleRunSimulatorCore.Tools.V3Dir(0)),
                    new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(x0 + radius, y0 + radius + d, 0), MarbleRunSimulatorCore.Tools.V3Dir(0), MarbleRunSimulatorCore.Tools.V3Dir(-90))
                ];
            }
            template.initialize();
            return template;
...
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class Ladder extends MarbleRunSimulatorCore.MachinePart {
        static _WallThickness = 0.005;
        static _WallDepth = 0.02;
        static _Drop = 0.01;
        leftWall;
        leftWallH;
        rightWall;
        ...
        constructor(machine, prop) {
            ...
            let x0 = -MarbleRunSimulatorCore.tileSize * 0.5;
            let x1 = -MarbleRunSimulatorCore.tileSize * 0.5 + prop.l * MarbleRunSimulatorCore.tileSize;
            let leftYMax = -MarbleRunSimulatorCore.tileHeight;
            let leftYMin = -prop.h * MarbleRunSimulatorCore.tileHeight + Ladder._Drop - 0.005;
            this.leftWallH = leftYMax - leftYMin;
            let rightYMax = 0;
            let rightYMin = -prop.h * MarbleRunSimulatorCore.tileHeight + MarbleRunSimulatorCore.tileHeight + Ladder._Drop - 0.005;
        ...
        static GenerateTemplate(l, h) {
            ...
            let dy = (h * MarbleRunSimulatorCore.tileSize - Ladder._Drop) / (count + 1);
            let angleDrop = Math.atan(Ladder._Drop / ((x1 - x0) / 6)) / Math.PI * 180 + 90;
            template.trackTemplates[0] = new MarbleRunSimulatorCore.TrackTemplate(template);
            template.trackTemplates[0].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(x0, 0, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(x1 - Ladder._WallThickness - hole, -Ladder._Drop, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
            ];
            template.trackTemplates[0].colorIndex = 0;
            template.trackTemplates[1] = new MarbleRunSimulatorCore.TrackTemplate(template);
            template.trackTemplates[1].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(x0 + Ladder._WallThickness, -h * MarbleRunSimulatorCore.tileHeight + Ladder._Drop, 0), MarbleRunSimulatorCore.Tools.V3Dir(angleDrop)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(x1, -h * MarbleRunSimulatorCore.tileHeight, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
            ];
            template.trackTemplates[1].colorIndex = 1;
            for (let n = 0; n < count; n++) {
                if (n % 2 === 0) {
                    let trackTemplate = new MarbleRunSimulatorCore.TrackTemplate(template);
                    trackTemplate.trackpoints = [
                        new MarbleRunSimulatorCore.TrackPoint(trackTemplate, new BABYLON.Vector3(x1 - Ladder._WallThickness + 0.002, 0 - dy * (n + 1), 0), MarbleRunSimulatorCore.Tools.V3Dir(-angleDrop)),
                        new MarbleRunSimulatorCore.TrackPoint(trackTemplate, new BABYLON.Vector3(x0 + Ladder._WallThickness + hole, 0 - dy * (n + 1) - Ladder._Drop, 0), MarbleRunSimulatorCore.Tools.V3Dir(-90)),
                    ];
                    ...
                    let trackTemplate = new MarbleRunSimulatorCore.TrackTemplate(template);
                    trackTemplate.trackpoints = [
                        new MarbleRunSimulatorCore.TrackPoint(trackTemplate, new BABYLON.Vector3(x0 + Ladder._WallThickness - 0.002, 0 - dy * (n + 1), 0), MarbleRunSimulatorCore.Tools.V3Dir(angleDrop)),
                        new MarbleRunSimulatorCore.TrackPoint(trackTemplate, new BABYLON.Vector3(x1 - Ladder._WallThickness - hole, 0 - dy * (n + 1) - Ladder._Drop, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                    ];
    ...
    MarbleRunSimulatorCore.Ladder = Ladder;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
/// <reference path="../machine/MachinePart.ts"/>
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
...
(function (MarbleRunSimulatorCore) {
    class Loop extends MarbleRunSimulatorCore.MachinePart {
        constructor(machine, prop, underLoop) {
            super(machine, prop);
            this.setColorCount(1);
            if (!isFinite(prop.n)) {
                prop.n = 1;
...
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class Diamond extends MarbleRunSimulatorCore.MachinePart {
        body;
        constructor(machine, prop) {
            super(machine, prop);
            this.setColorCount(1);
            this.setTemplate(this.machine.templateManager.getTemplate(Diamond.PropToPartName(prop), prop.mirrorX));
            let s = prop.l * MarbleRunSimulatorCore.tileSize / Math.SQRT2;
            ...
            let bodyVertexData = Mummu.CreateBeveledBoxVertexData({ width: s, height: s, depth: MarbleRunSimulatorCore.tileSize });
    ...
    MarbleRunSimulatorCore.Diamond = Diamond;
    class Box extends MarbleRunSimulatorCore.MachinePart {
        body;
        constructor(machine, prop) {
            super(machine, prop);
            this.setColorCount(1);
            this.setTemplate(this.machine.templateManager.getTemplate(Box.PropToPartName(prop), prop.mirrorX));
            let w = prop.l * MarbleRunSimulatorCore.tileSize;
    ...
    MarbleRunSimulatorCore.Box = Box;
    class Bumper extends MarbleRunSimulatorCore.MachinePart {
        body;
        constructor(machine, prop) {
            super(machine, prop);
            this.setColorCount(1);
            this.setTemplate(this.machine.templateManager.getTemplate(Bumper.PropToPartName(prop), prop.mirrorX));
            let s = prop.l * MarbleRunSimulatorCore.tileSize;
            this.body = new BABYLON.Mesh("body");
            this.body.parent = this;
            if (prop.l % 2 === 0) {
                this.body.position.x = MarbleRunSimulatorCore.tileSize * 0.5;
            ...
            let bodyVertexData = BABYLON.CreateCylinderVertexData({ diameter: s, height: MarbleRunSimulatorCore.tileSize });
    ...
    MarbleRunSimulatorCore.Bumper = Bumper;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
...
(function (MarbleRunSimulatorCore) {
    class Screen extends MarbleRunSimulatorCore.MachinePart {
        _animatePivot = Mummu.AnimationFactory.EmptyNumberCallback;
        _animateLock0 = Mummu.AnimationFactory.EmptyNumberCallback;
        _animateLock2 = Mummu.AnimationFactory.EmptyNumberCallback;
        _animateTingle2Out = Mummu.AnimationFactory.EmptyNumberCallback;
        container;
        pixels = [];
        pixelPictures = [];
        lock0;
...
(function (MarbleRunSimulatorCore) {
    class Shooter extends MarbleRunSimulatorCore.MachinePart {
        ...
        clicSound;
        base;
        animateKickerArm = Mummu.AnimationFactory.EmptyNumberCallback;
        animateKickerKick = Mummu.AnimationFactory.EmptyNumberCallback;
        constructor(machine, prop) {
            super(machine, prop);
            this.setColorCount(5);
...
(function (MarbleRunSimulatorCore) {
    class Snake extends MarbleRunSimulatorCore.MachinePart {
        ...
        static GenerateTemplate(l, s) {
            ...
            let z0 = 0;
            template.trackTemplates[0] = new MarbleRunSimulatorCore.TrackTemplate(template);
            let start = new BABYLON.Vector3(-MarbleRunSimulatorCore.tileSize * 0.5, 0, z0);
            let end = new BABYLON.Vector3(MarbleRunSimulatorCore.tileSize * (template.l - 0.5), 0, z0);
            ...
            for (let i = 1; i < count; i++) {
                let x = -MarbleRunSimulatorCore.tileSize * 0.5 + i * r;
                if (i === 1) {
                    let z = z0 - r;
...
(function (MarbleRunSimulatorCore) {
    class Speeder extends MarbleRunSimulatorCore.MachinePart {
        ...
        constructor(machine, prop) {
            ...
            this.base = new BABYLON.Mesh("base");
            this.base.material = this.game.materials.getMaterial(this.getColor(0), this.machine.materialQ);
            this.base.position.x = (-MarbleRunSimulatorCore.tileSize * 0.5 + MarbleRunSimulatorCore.tileSize * (prop.l - 0.5)) * 0.5;
            this.base.parent = this;
            this.wheel0 = new BABYLON.Mesh("wheel0");
            this.wheel0.parent = this;
            this.wheel0.position.x = (-MarbleRunSimulatorCore.tileSize * 0.5 + MarbleRunSimulatorCore.tileSize * (prop.l - 0.5)) * 0.5;
            this.wheel0.position.y = 0.006;
            this.wheel0.position.z = -0.008 - 0.006;
            this.wheel1 = new BABYLON.Mesh("wheel1");
            this.wheel1.parent = this;
            this.wheel1.position.x = (-MarbleRunSimulatorCore.tileSize * 0.5 + MarbleRunSimulatorCore.tileSize * (prop.l - 0.5)) * 0.5;
...
(function (MarbleRunSimulatorCore) {
    class Start extends MarbleRunSimulatorCore.MachinePart {
        ...
        static GenerateTemplate() {
            ...
            template.trackTemplates[0].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileWidth * 0.5, 0, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(MarbleRunSimulatorCore.tileWidth * 0.5, 0.008, 0), MarbleRunSimulatorCore.Tools.V3Dir(70))
            ];
            template.trackTemplates[0].drawEndTip = true;
            template.initialize();
            return template;
...
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class TJoin extends MarbleRunSimulatorCore.MachinePart {
        constructor(machine, prop) {
            super(machine, prop);
            this.setColorCount(3);
        ...
        static GenerateTemplate() {
            ...
            template.trackTemplates[2].colorIndex = 2;
            template.trackTemplates[2].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[2], new BABYLON.Vector3(0, 0, MarbleRunSimulatorCore.tileSize * 0.5), new BABYLON.Vector3(0, -1, 0), new BABYLON.Vector3(0, 0, -1)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[2], new BABYLON.Vector3(0, -MarbleRunSimulatorCore.tileHeight, -MarbleRunSimulatorCore.tileWidth * 0.5), new BABYLON.Vector3(0, 0, -1))
            ];
            template.trackTemplates[3] = new MarbleRunSimulatorCore.TrackTemplate(template);
            template.trackTemplates[3].colorIndex = 3;
            template.trackTemplates[3].trackpoints = [
    ...
    MarbleRunSimulatorCore.TJoin = TJoin;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class TSplit extends MarbleRunSimulatorCore.MachinePart {
        _animatePivot = Mummu.AnimationFactory.EmptyNumberCallback;
        anchor;
        pivot;
        axisZMin = 0;
        ...
        static GenerateTemplate(mirror) {
            ...
            template.trackTemplates[0].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileWidth * 0.5, 0, 0), dir),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], pEndLeft, dirEnd)
            ];
            template.trackTemplates[1] = new MarbleRunSimulatorCore.TrackTemplate(template);
            template.trackTemplates[1].preferedStartBank = 0;
            template.trackTemplates[1].preferedEndBank = 0;
            template.trackTemplates[1].colorIndex = 1;
            template.trackTemplates[1].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(0, -MarbleRunSimulatorCore.tileHeight * 0.5 - MarbleRunSimulatorCore.Split.pivotL / Math.SQRT2 - 0.0005, MarbleRunSimulatorCore.Split.pivotL / Math.SQRT2), Mummu.RotateInPlace(MarbleRunSimulatorCore.Tools.V3Dir(115), BABYLON.Axis.Y, -Math.PI * 0.5), BABYLON.Vector3.Up()),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[1], new BABYLON.Vector3(0, -MarbleRunSimulatorCore.tileHeight * template.h, 0.5 * MarbleRunSimulatorCore.tileWidth), new BABYLON.Vector3(0, 0, 1), BABYLON.Vector3.Up())
            ];
            template.trackTemplates[2] = new MarbleRunSimulatorCore.TrackTemplate(template);
            template.trackTemplates[2].preferedStartBank = 0;
            template.trackTemplates[2].colorIndex = 2;
            template.trackTemplates[2].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[2], new BABYLON.Vector3(0, -MarbleRunSimulatorCore.tileHeight * 0.5 - MarbleRunSimulatorCore.Split.pivotL / Math.SQRT2 - 0.0005, -MarbleRunSimulatorCore.Split.pivotL / Math.SQRT2), Mummu.RotateInPlace(MarbleRunSimulatorCore.Tools.V3Dir(115), BABYLON.Axis.Y, Math.PI * 0.5), BABYLON.Vector3.Up()),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[2], new BABYLON.Vector3(0, -MarbleRunSimulatorCore.tileHeight * template.h, -0.5 * MarbleRunSimulatorCore.tileWidth), new BABYLON.Vector3(0, 0, -1), BABYLON.Vector3.Up())
            ];
            template.trackTemplates[3] = new MarbleRunSimulatorCore.TrackTemplate(template);
            template.trackTemplates[3].preferedStartBank = 0;
            template.trackTemplates[3].colorIndex = 3;
            template.trackTemplates[3].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[3], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileWidth * 0.5 + 0.01, 0.016, 0), dir, BABYLON.Vector3.Down()),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[3], pEndLeft.add(new BABYLON.Vector3(0.019, 0, 0)), BABYLON.Vector3.Down(), BABYLON.Vector3.Left()),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[3], pEndLeft.add(new BABYLON.Vector3(0.019, -0.008, 0)), BABYLON.Vector3.Down(), BABYLON.Vector3.Left())
            ];
            template.trackTemplates[3].drawStartTip = true;
    ...
    MarbleRunSimulatorCore.TSplit = TSplit;
})(MarbleRunSimulatorCore || (MarbleRunSimulatorCore = {}));
var MarbleRunSimulatorCore;
(function (MarbleRunSimulatorCore) {
    class TeardropTurn extends MarbleRunSimulatorCore.MachinePart {
        ...
        static GenerateTemplate(h) {
            ...
            let r2 = r / Math.SQRT2;
            let cX = 3 * MarbleRunSimulatorCore.tileSize;
            ...
            template.trackTemplates[0].trackpoints = [
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileSize * 0.5, 0, 0), MarbleRunSimulatorCore.Tools.V3Dir(90)),
                new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(cX, MarbleRunSimulatorCore.tileHeight * 0.5, r), (new BABYLON.Vector3(1, 0.1, 0)).normalize()),
            ...
            for (let h = 1; h < template.h; h++) {
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(cX - r2, MarbleRunSimulatorCore.tileHeight * 0.5, -r2)));
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(cX - r, MarbleRunSimulatorCore.tileHeight * 0.5, 0)));
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(cX - r2, MarbleRunSimulatorCore.tileHeight * 0.5, r2)));
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(cX, MarbleRunSimulatorCore.tileHeight * 0.5, r)));
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(cX + r2, MarbleRunSimulatorCore.tileHeight * 0.5, r2)));
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(cX + r, MarbleRunSimulatorCore.tileHeight * 0.5, 0)));
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(cX + r2, MarbleRunSimulatorCore.tileHeight * 0.5, -r2)));
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(cX, MarbleRunSimulatorCore.tileHeight * 0.5, -r), (new BABYLON.Vector3(-1, 0.1, 0)).normalize()));
            }
            ...
            template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileSize * 0.5, MarbleRunSimulatorCore.tileHeight, 0), MarbleRunSimulatorCore.Tools.V3Dir(-90)));
            template.trackTemplates[0].onNormalEvaluated = (n) => {
                n.copyFromFloats(0, 1, 0);
            };
            let hermite = (x) => {
        ...
        static GenerateTemplate(h, pipeVersion, woodVersion) {
            ...
            if (pipeVersion) {
                ...
                for (let a = 1; a < 8; a++) {
                    ...
                    let cosa = Math.cos(angle);
                    let sina = Math.sin(angle);
                    let dir = MarbleRunSimulatorCore.Tools.V3Dir(90 - angle / Math.PI * 180);
                    let norm = new BABYLON.Vector3(0, 0, 1);
                    let p = new BABYLON.Vector3(0, -r, 0);
                    Mummu.RotateInPlace(p, BABYLON.Axis.Z, angle);
                    p.x += -MarbleRunSimulatorCore.tileWidth * 0.5 + 0.005;
                    p.y += cY;
                    template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], p, dir, norm));
                }
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(-MarbleRunSimulatorCore.tileWidth * 0.5, yTop, 0), MarbleRunSimulatorCore.Tools.V3Dir(-90), MarbleRunSimulatorCore.Tools.V3Dir(0)));
            }
            else {
                template.trackTemplates[0] = new MarbleRunSimulatorCore.TrackTemplate(template);
                template.trackTemplates[0].trackpoints = [
    ...
    class Wall extends MarbleRunSimulatorCore.MachinePart {
        ...
        static GenerateTemplate(l, h) {
            ...
            for (let n = 0; n < N; n++) {
                ...
                template.trackTemplates[0].trackpoints.push(new MarbleRunSimulatorCore.TrackPoint(template.trackTemplates[0], new BABYLON.Vector3(x, y, z), dir));
            }
            for (let n = 0; n <= N; n++) {
                let f = n / N;
                f = 1 - f;
            ...
            let c = new BABYLON.Vector3(-MarbleRunSimulatorCore.tileSize * 0.5, MarbleRunSimulatorCore.tileHeight * template.h * 0.25, MarbleRunSimulatorCore.tileSize * template.l * 0.5);
...
(function (MarbleRunSimulatorCore) {
    class QuarterNote extends MarbleRunSimulatorCore.MachinePart {
        static NoteNames = ["c3", "d3", "e3", "f3", "g3", "a4", "b4", "c4"];
        static index = 0;
        notes = [];
        tings = [];
        noteMesh = [];
...
(function (MarbleRunSimulatorCore) {
    ...
    class Room {
        ...
        async instantiateSimple(groundColor, wallColor, size = 10) {
            this.decors.forEach(decor => {
                decor.dispose();
            });
            this.decors = [];
        ...
        async instantiateBBPuzzle(skyboxPath) {
            this.decors.forEach(decor => {
                decor.dispose();
            });
            this.decors = [];
