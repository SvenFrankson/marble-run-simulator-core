...
namespace MarbleRunSimulatorCore {
    ...
    export class Pixel extends MachinePart {
        ...
        constructor(machine: Machine, prop: IMachinePartProp) {
            ...
            this.screenMesh = Mummu.CreateBeveledBox(
                "screen",
                {
                    width: 2 * tileSize - 0.002,
                    height: 2 * tileHeight - 0.002,
                    depth: 0.003,
                }
            );
            this.screenMesh.position.x = tileSize * 0.5;
            this.screenMesh.position.y = - tileHeight * 0.5;
            this.screenMesh.position.z = tileSize * 0.5;
        ...
        public static GenerateTemplate(): MachinePartTemplate {
            ...
            template.trackTemplates[1].trackpoints = [
                new TrackPoint(
                    ...
                    new BABYLON.Vector3(r, - tileHeight + 0.004, 0),
                    (new BABYLON.Vector3(-1, -0.4, 0)).normalize()
                ),
                new TrackPoint(
                    template.trackTemplates[1],
                    new BABYLON.Vector3(- tileSize * 0.5, - tileHeight, 0),
                    new BABYLON.Vector3(-1, 0, 0)
                )
            ];
